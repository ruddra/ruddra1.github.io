...
Django has its own implementation for `reset/forgot password` for its admin site. We are going to use that piece of code as reference to implement simillar feature for a non admin-site authentication page. Although there are tons of good packages which will allow user to use their password reseting system. But if the system isn't too complex and doesn't need such authentication pluggins, then reusing the django's very own implementation can be a good option.<!--more-->
